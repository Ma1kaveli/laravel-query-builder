...
class ApplyDateNotRangeTest extends TestCase
{
    /**
     ...
     */
    public function test_apply_calls_where_not_between_with_and_logic()
    {
        $this->setDateTimeFormat();

        $builder = Mockery::mock(EloquentBuilder::class);

        $filter = Mockery::mock(ApplyDateNotRange::class)
            ->makePartial()
        ...
        $builder->shouldReceive('whereNotBetween')
            ->once()
            ->with(
                'users.created_at',
                ['2024-01-01', '2024-01-31'],
     ...
     * OR логика
     */
    public function test_apply_calls_where_not_between_with_or_logic()
    {
        $this->setDateTimeFormat();
