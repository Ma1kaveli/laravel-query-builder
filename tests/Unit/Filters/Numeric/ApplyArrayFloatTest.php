...
class ApplyArrayFloatTest extends TestCase
{
    /**
    ...
    public function test_apply_returns_early_if_field_is_not_string()
    {
        ...
        $filter = new ApplyArrayFloat();
        $filter->apply($builder, null, [1.1, 2.2], []);
        $this->assertTrue(true);
    }

    /**
     * value не float → ранний return
     */
    public function test_apply_returns_early_if_value_is_not_float_array()
    {
        $builder = Mockery::mock(EloquentBuilder::class);
        $builder->shouldNotReceive('whereIn');
    ...
    /**
     * value пустой массив → ранний return
     */
    ...
    {
        $builder = Mockery::mock(EloquentBuilder::class);
        $filter = Mockery::mock(ApplyArrayFloat::class)->makePartial()->shouldAllowMockingProtectedMethods();
