...
class ApplyIntegerTest extends TestCase
{
    /**
    ...
    public function test_apply_returns_early_if_field_is_not_string()
    {
        ...
        $filter = new ApplyInteger();
        $filter->apply($builder, null, 1, []);
        $this->assertTrue(true);
    }

    /**
     * Значение не integer → ранний return
     */
    ...
    {
        $builder = Mockery::mock(EloquentBuilder::class);
        $filter = Mockery::mock(ApplyInteger::class)->makePartial()->shouldAllowMockingProtectedMethods();

        $filter->shouldReceive('getFieldWithTable')->once()->with($builder, 'field')->andReturn('table.field');
        $builder->shouldReceive('where')->once()->with('table.field', 10, null, 'and');
    ...
    public function test_apply_calls_where_with_or_logic()
    {
        ...
        $filter->shouldReceive('getFieldWithTable')->once()->with($builder, 'field')->andReturn('table.field');
        $builder->shouldReceive('where')->once()->with('table.field', 10, null, 'or');
