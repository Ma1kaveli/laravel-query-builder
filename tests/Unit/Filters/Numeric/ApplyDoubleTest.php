...
class ApplyDoubleTest extends TestCase
{
    /**
    ...
    public function test_apply_returns_early_if_field_is_not_string()
    {
        ...
        $filter = new ApplyDouble();
        $filter->apply($builder, null, 1.1, []);
        $this->assertTrue(true);
    }

    /**
     * Значение не double → ранний return
     */
    public function test_apply_returns_early_if_value_is_not_double()
    {
        $builder = Mockery::mock(EloquentBuilder::class);
        $builder->shouldNotReceive('where');
    ...
    {
        $builder = Mockery::mock(EloquentBuilder::class);
        $filter = Mockery::mock(ApplyDouble::class)->makePartial()->shouldAllowMockingProtectedMethods();
